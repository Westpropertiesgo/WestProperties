...
interface HeaderProps {
  /**
   * Pages whose content starts with something other than a dark hero
   * (i.e. almost every page except the homepage, /sell, /contact, and
   * /insider) need the header to render in its solid/dark-text state from
   * the very first frame — otherwise transparent white-on-white text is
   * shown until the user scrolls past 24px. Scroll position alone can't
   * tell the header what's behind it, so each page says so explicitly.
   */
  alwaysSolid?: boolean;
}
...
export default function Header({ alwaysSolid = false }: HeaderProps = {}) {
  ...
  // The single source of truth for "does the header need its solid,
  // dark-text appearance right now" — either the page told us its top
  // section isn't a dark hero, or the user has actually scrolled past it.
    ...
    <>
      <header
        className={`fixed inset-x-0 top-0 z-50 transition-all duration-500 ease-signature ${
          solid ? "bg-ivory/95 shadow-[0_1px_0_0_rgba(20,33,43,0.08)] backdrop-blur" : "bg-transparent"
        }`}
      >
        <div className="mx-auto flex h-20 max-w-8xl items-center justify-between px-6 md:h-24 md:px-10 lg:pl-8 lg:pr-6">
          <div className="flex items-center gap-2 md:gap-4">
            ...
            <button
              ...
              className={`transition-colors duration-300 ${solid ? "text-ink" : "text-ivory"}`}
            >
              ...
              {navLinks.map((link) => (
                <Link
                  ...
                  className={`font-mono text-[12px] uppercase tracking-widest2 transition-colors duration-300 ${
                    solid ? "text-ink/70 hover:text-brass" : "text-ivory/85 hover:text-brass-light"
                  }`}
                >
                  {link.label}
                </Link>
              ...
              <div className="relative" ref={toolsRef}>
                <button
                  ...
                  className={`flex items-center gap-1.5 font-mono text-[12px] uppercase tracking-widest2 transition-colors duration-300 ${
                    solid ? "text-ink/70 hover:text-brass" : "text-ivory/85 hover:text-brass-light"
                  } ${toolsOpen ? (solid ? "text-brass" : "text-brass-light") : ""}`}
                >
                  Tools
                  <CaretIcon />
                </button>
      ...
      {/* Rendered as siblings of <header>, not children — the header gains
          backdrop-blur once solid, and backdrop-blur establishes a new
          containing block for fixed-position descendants. Nesting these
          inside <header> squashed them into the header's own ~80px box
          instead of the viewport the instant the header went solid, which
          is exactly why the menu only opened correctly at the very top of
          the page. */}
      <NavDrawer
        open={drawerOpen}
